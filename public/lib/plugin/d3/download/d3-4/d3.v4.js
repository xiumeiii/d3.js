...
// See https://www.particleincell.com/2012/bezier-splines/ for derivation.
    ...
    var interpolateString = function(a, b) {
        var bi = reA.lastIndex = reB.lastIndex = 0, // scan index for next number in b
            ...
            i = -1, // index in s
            s = [], // string constants and placeholders  字符串常量和占位符
            q = []; // number interpolators

        // Coerce inputs to strings.   强制输入字符串。
        a = a + "", b = b + "";

        // Interpolate pairs of numbers in a & b.   在a＆b中插入数对。
        while ((am = reA.exec(a))
        && (bm = reB.exec(b))) {
        ...
        // Add remains of b.  添加b的剩余。
        if (bi < b.length) {
            bs = b.slice(bi);
            if (s[i]) s[i] += bs; // coalesce with previous string 与以前的字符串合并
            else s[++i] = bs;
        }

        // Special optimization for only a single match. 特殊的优化只有一个匹配。
        // Otherwise, interpolate each of the numbers and rejoin the string.  否则，插入每个数字并重新加入字符串
